...
def plot_reconstruction(
    ...
    # Trích min/max một cách an toàn cho mọi phiên bản pycolmap
    if isinstance(bbs, tuple) or isinstance(bbs, list):
        bb_min, bb_max = bbs
    else:
        # Có thể là đối tượng AlignedBox3d hoặc custom class
        bb_min = bbs.min if isinstance(bbs.min, np.ndarray) else bbs.min()
        bb_max = bbs.max if isinstance(bbs.max, np.ndarray) else bbs.max()

    p3Ds = [
        p3D for _, p3D in rec.points3D.items()
        if (
            (p3D.xyz >= bb_min).all()
            and (p3D.xyz <= bb_max).all()
