...
def main(conf: Dict,
    ...
    model = Model(conf['model']).eval().to(torch.float32).to(device)

    loader = torch.utils.data.DataLoader(
        dataset, num_workers=1, shuffle=False, pin_memory=True)
    for idx, data in enumerate(tqdm(loader)):
        name = dataset.names[idx]
        pred = model({'image': data['image'].to(torch.float32).to(device, non_blocking=True)})
        pred = {str(k): v[0].cpu().numpy() for k, v in pred.items()}
